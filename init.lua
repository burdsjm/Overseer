...
do
  ...
  logger.info("\agWelcome to Overseer Commander!")
  logger.info('\aoType \at/mqoverseer help \aofor commands.')
end
...
if not package.loaded['overseer_events_initialized'] then
   local function try_register(name, pattern, callback)
      local ok, err = pcall(function() mq.event(name, pattern, callback) end)
      if ok then
         return true
      end

      -- tolerate "already defined" message from mq.event (some MQ implementations return that)
      if tostring(err):match("already defined") then
         return true
      end

      -- unexpected error: rethrow so you notice it during startup
      error(err)
   end

   -- Register events (these call the Event_* functions defined by require('overseer'))
   try_register('NoActiveQuests', '#*#You currently have 5 active Overseer quests #*#', Event_NoActiveQuests)
   try_register('TooManyRewards', '#*#You have too many pending rewards to claim another!#*#', Event_TooManyRewards)
   try_register('FailedStart', '#*#has failed to start. Please verify your#*#', Event_FailedStart)
   try_register('NoPendingRewards', '#*#You currently do not have any pending rewards.#*#', Event_NoPendingRewards)
   try_register('ErrorGrantingReward', '#*#The system is currently unable to grant your reward for#*#', Event_ErrorGrantingReward)
